...
class Agent_DDPG(object):
    def __init__(self, action_size, state_size, action_limit, ):
        ...
        # Maximize Q 를 해야하므로 learning rate에 '-' 를 붙인다. 업데이트 할 파라미터를 var_list를 사용해서 명시할 수 있다.
        ...
        self.ctrain = tf.train.AdamOptimizer(self.critic_learning_rate).minimize(self.mseloss,
                                                                                 var_list=self.critic_evaluation_params)
    ...
    def choose_action(self, s):
        return np.clip(
            np.random.normal(self.sess.run(self.a, {self.input_state: s[np.newaxis, :]})[0], self.action_variance), -2,
            2)
    ...
    def learn(self):
        if self.replayBuffer.count() > self.batch_size:
            ...
            critic_loss, _ = self.sess.run([self.mseloss, self.ctrain],
                                           {self.input_state: batch_s, self.a: batch_a, self.R: batch_r,
    ...
    def plot_loss(self):
        ...
        plt.plot(np.arange(len(self.actor_loss_history)), self.actor_loss_history, '-p', color='b', markevery=me,
                 label=r'actor loss', lw=line_width,
                 markersize=ms)
        plt.plot(np.arange(len(self.critic_loss_history)), self.critic_loss_history, '--^', color='r', markevery=me,
